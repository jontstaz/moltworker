...
export async function syncToR2(sandbox: Sandbox, env: MoltbotEnv): Promise<SyncResult> {
  ...
  try {
    ...
    if (lastSync && lastSync.match(/^\d{4}-\d{2}-\d{2}/)) {
      // Clean up legacy clawdbot/ directory if it still exists (after successful sync)
      // This removes old data that has already been migrated to openclaw/
      try {
        await sandbox.startProcess(`rm -rf ${R2_MOUNT_PATH}/clawdbot 2>/dev/null || true`);
      } catch {
