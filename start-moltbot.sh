...
# ============================================================
# MIGRATE FROM LEGACY CLAWDBOT STRUCTURE
# ============================================================
# If old clawdbot/ directory exists but openclaw/ doesn't, migrate it
if [ -d "$BACKUP_DIR/clawdbot" ] && [ ! -d "$BACKUP_DIR/openclaw" ]; then
  echo "Migrating legacy clawdbot/ directory to openclaw/..."
  mv "$BACKUP_DIR/clawdbot" "$BACKUP_DIR/openclaw"
  echo "Migration complete: clawdbot/ -> openclaw/"
fi

# Also migrate legacy clawdbot.json file if it exists at root
if [ -f "$BACKUP_DIR/clawdbot.json" ] && [ ! -f "$BACKUP_DIR/openclaw/openclaw.json" ]; then
